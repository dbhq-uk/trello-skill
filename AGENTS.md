...
- Every change a SKILL.md asks for has a verb in a script. When a skill needs a new write - life-manager's board, list and label setup, or ticking a checklist item - add the verb to `trello-boards.sh` or `trello-cards.sh` and document it in trello/SKILL.md. A SKILL.md never has the agent build a request of its own, because that puts the key and token on the agent's command line.
- A script that writes many cards writes only the ones that must change, checks every write, and stops at the first one Trello refuses, naming the card and saying the rest was not done. `life-board.sh sort --apply` is the model: a sorted list gets no write, because every write moves a card's `dateLastActivity` and fills the board's activity.
