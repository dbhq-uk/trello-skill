...
- Shell scripts use `set -e`; errors go to stderr, structured output to stdout. `api()` in lib.sh stops the script on any non-2xx answer with Trello's status and message, because Trello sends its errors as plain text, not JSON. So a verb handles success only, and prints "nothing found" only for a real empty result.
